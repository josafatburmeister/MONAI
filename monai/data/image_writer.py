...
class ImageWriter:
    ...
    def write(cls, filename_or_obj, data_obj, **kwargs):
        ...
        raise NotImplementedError(f"Subclass of {cls.__name__} must implement this backend-specific method.")

    @classmethod
    def create_data_obj(cls, data_array, affine, channel_dim, dtype, **kwargs):
    ...
    def data_obj(
        ...
        Args:
            ...
            kwargs: additional keyword arguments to pass to :py:func:`cls.convert_to_target_affine`.
            obj_kwargs: additional keyword arguments to pass to :py:func:`cls.convert_to_target_affine`.
        ...
        # prepare data array
        data_array = cls.convert_to_channel_last(data_array, channel_dim=channel_dim, squeeze_end_dims=squeeze_end_dims)
        ...
        return cls.create_data_obj(
            data_array=data_array, affine=affine, channel_dim=channel_dim, dtype=output_dtype, **obj_kwargs
        )
...
class ITKWriter(ImageWriter):
    ...
    @classmethod
    def create_data_obj(cls, data_array, channel_dim, affine, dtype, **_kwargs):
        """create an ITK object from ``data_array``."""
...
class NibabelWriter(ImageWriter):
    ...
    @classmethod
    def create_data_obj(cls, data_array, dtype=None, **kwargs):
        """create a Nifti1Image object from ``data_array``."""
        if dtype is not None:
            data_array = data_array.astype(dtype, copy=False)
        return Nifti1Image(data_array, affine, **kwargs)

    @classmethod
    def write(cls, filename_or_obj, data_obj, **_kwargs):
